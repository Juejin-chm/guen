<template>
	<view>
		...
		<view class="poster">
			...
			<view class="number">
				<view class="b">{{data.user_promo_code}}</view>
				<view>我的推广码</view>
			</view>
			<view class="code">
				<image :src="data.file_path" mode=""></image>
				<view>长按识别二维码</view>
			</view>
...
<script>
	export default {
		...
		onLoad() {
			this.$api('/user-share-page').then(({
				data
			}) => {
				this.data = data
				console.log(data.user_promo_code, data.file_path, '-----------');
				this.getCanvas(data.user_promo_code, data.file_path)
			})
		},
		methods: {
			getImageInfo(imgSrc) {
			...
			async getCanvas(codeNumber, qrCodePath) {
